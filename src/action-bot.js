...
class ActionBot {
    ...
    async onMessageReceived(msg) {
        ...
        const systemMessage = `[Current time: ${new Date().toLocaleString()} pacific]\n\n` +
    ...
    async shouldRespondToMessage(msg, client) {
        if (msg.author.id === client.user.id) {
            return false;
        }
        if (!msg.channel.isTextBased()) {
        ...
        try {
            ...
            if (msg.guild && !isMentioned) {
                ...
                let context = (await channel.messages
                    .fetch({ limit: 10 }))
                    .reverse()
 ...
 */
function formatMessageSimple(msg, client) {
...
function formatMessage(msg, client, includeChannel = true) {
    const channelName = msg.ChannelType == discord_js_1.ChannelType.DM ? 'Direct Message' : `#${msg.channel.name}`;
